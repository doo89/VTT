...
if (headerIdx !== -1) {
    ...
    // Back track to find the opening div of the grid
    let divStartIdx = startIdx;
    while (divStartIdx > headerIdx && !lines[divStartIdx].includes('<div className="grid')) divStartIdx--;
    ...
    let divEndIdx = endIdx;
    while (divEndIdx < lines.length && !lines[divEndIdx].includes('</div>')) divEndIdx++;
    ...
    if (divStartIdx !== -1 && divEndIdx !== -1) {
        ...
        lines.splice(divStartIdx, divEndIdx - divStartIdx + 1, newLayout);
        fs.writeFileSync(path, lines.join('\n'));
        console.log('Successfully redesigned Badge/Corner settings');
    }
}
